...
async def run_agent_loop(
    ...
    model = "gemini-3-flash-preview"  # Always use the state-of-the-art model for v2
    ...
    try:
        while step < max_steps:
            ...
            # Send current chat state to Gemini 3 with tool definitions (with robust dynamic backoff for 429 errors)
            max_retries = 5
            retry_delay = 5.0
            response = None
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if is_rate_limit and attempt < max_retries:
                        ...
                        # Extract exact retry duration if specified in error message
                        match = re.search(r"retry in ([\d\.]+)s", err_msg)
                        if match:
                            sleep_time = float(match.group(1)) + 1.5  # 1.5s extra padding
                        else:
                            match_retry = re.search(r"retryDelay': '(\d+)s'", err_msg)
                            if match_retry:
                                sleep_time = float(match_retry.group(1)) + 1.5
                        
                        print(f"[Agent] Rate limit hit (429). Retrying in {sleep_time:.2f}s... (Attempt {attempt + 1}/{max_retries})")
